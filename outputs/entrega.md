# Nota sobre prefijos de rol
Se emplean prefijos explícitos `System:` y `User:` en los prompts para diferenciar instrucciones del orquestador (reglas, restricciones, formato) frente a las solicitudes del analista. Esta convención evita que el modelo confunda roles, reduce la ambigüedad en la responsabilidad de cada instrucción y previene que contenido sensible del sistema se mezcle con ejemplos de usuario.
...
Técnica 2: Roles explícitos + formato JSON obligatorio; se fijan instrucciones con roles system/user para validar que el LLM devuelva artefactos listos (OpenAPI, DTOs, validaciones ACID cuando aplique) y evita omisiones.
